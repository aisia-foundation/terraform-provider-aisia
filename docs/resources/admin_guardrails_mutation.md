...
page_title: "aisia_admin_guardrails_mutation Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation PUT exacte /admin/guardrails (resource action Terraform).
...
# aisia_admin_guardrails_mutation (Resource)
...
resource "aisia_admin_guardrails_mutation" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_guardrails_mutation.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_guardrails_mutation` — synchronisée avec l'OpenAPI AISIA.
