...
page_title: "aisia_org_webhooks_wh_id_mutation Resource - aisia"
subcategory: ""
description: |-
  Endpoint API AISIA — mutation DELETE exacte /org/webhooks/{wh_id} (resource action Terraform).
  Endpoint : /org/webhooks/{wh_id}Documentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_org_webhooks_wh_id_mutation (Resource)

Endpoint API AISIA — mutation DELETE exacte `/org/webhooks/{wh_id}` (resource action Terraform).

- **Endpoint** : `/org/webhooks/{wh_id}`
...
resource "aisia_org_webhooks_wh_id_mutation" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_org_webhooks_wh_id_mutation.run.json)
}
```
...
> Ressource `resource` : `aisia_org_webhooks_wh_id_mutation` — synchronisée avec l'OpenAPI AISIA.
