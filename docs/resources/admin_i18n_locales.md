...
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/i18n/locales (resource action Terraform).
...
```terraform
resource "aisia_admin_i18n_locales" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_i18n_locales.example.id
}
```
...
> Ressource `resource` : `aisia_admin_i18n_locales` — synchronisée avec l'OpenAPI AISIA.
