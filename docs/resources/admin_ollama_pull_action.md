...
page_title: "aisia_admin_ollama_pull_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/ollama/pull (resource action Terraform).
  Endpoint : /admin/ollama/pullDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_ollama_pull_action (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/ollama/pull` (resource action Terraform).

- **Endpoint** : `/admin/ollama/pull`
...
resource "aisia_admin_ollama_pull_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_ollama_pull_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_ollama_pull_action` — synchronisée avec l'OpenAPI AISIA.
