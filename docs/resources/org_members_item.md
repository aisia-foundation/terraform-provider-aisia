...
page_title: "aisia_org_members_item Resource - aisia"
subcategory: ""
description: |-
  Endpoint API AISIA — mutation PUT exacte /org/members/{user_id} (resource action Terraform).
...
# aisia_org_members_item (Resource)
...
```terraform
resource "aisia_org_members_item" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_org_members_item.example.id
}
```
...
> Ressource `resource` : `aisia_org_members_item` — synchronisée avec l'OpenAPI AISIA.
