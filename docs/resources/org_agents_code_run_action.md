...
page_title: "aisia_org_agents_code_run_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint API AISIA — mutation POST exacte /org/agents/{code}/run (resource action Terraform).
  Endpoint : /org/agents/{code}/runDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_org_agents_code_run_action (Resource)

Endpoint API AISIA — mutation POST exacte `/org/agents/{code}/run` (resource action Terraform).

- **Endpoint** : `/org/agents/{code}/run`
...
resource "aisia_org_agents_code_run_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_org_agents_code_run_action.run.json)
}
```
...
> Ressource `resource` : `aisia_org_agents_code_run_action` — synchronisée avec l'OpenAPI AISIA.
