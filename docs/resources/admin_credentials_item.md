...
page_title: "aisia_admin_credentials_item Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation PUT exacte /admin/credentials/{provider} (resource action Terraform).
...
# aisia_admin_credentials_item (Resource)
...
```terraform
resource "aisia_admin_credentials_item" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_credentials_item.example.id
}
```
...
> Ressource `resource` : `aisia_admin_credentials_item` — synchronisée avec l'OpenAPI AISIA.
