...
page_title: "aisia_admin_bot_run_now_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/bot/run-now (resource action Terraform).
  Endpoint : /admin/bot/run-nowDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_bot_run_now_action (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/bot/run-now` (resource action Terraform).

- **Endpoint** : `/admin/bot/run-now`
...
resource "aisia_admin_bot_run_now_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_bot_run_now_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_bot_run_now_action` — synchronisée avec l'OpenAPI AISIA.
