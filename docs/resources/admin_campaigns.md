...
page_title: "aisia_admin_campaigns Resource - aisia"
subcategory: ""
description: |-
...
# aisia_admin_campaigns (Resource)
...
```terraform
resource "aisia_admin_campaigns" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_campaigns.example.id
}
```
...
- `channel` (String) champ channel
- `content` (String) champ content
...
- `target_audience` (String) champ target_audience
...
> Ressource `resource` : `aisia_admin_campaigns` — synchronisée avec l'OpenAPI AISIA.
