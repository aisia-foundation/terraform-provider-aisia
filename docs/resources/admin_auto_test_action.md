...
page_title: "aisia_admin_auto_test_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/auto-test (resource action Terraform).
...
# aisia_admin_auto_test_action (Resource)
...
resource "aisia_admin_auto_test_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_auto_test_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_auto_test_action` — synchronisée avec l'OpenAPI AISIA.
