...
page_title: "aisia_admin_token_quotas Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/token-quotas (resource action Terraform).
  Endpoint : /admin/token-quotasDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_token_quotas (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/token-quotas` (resource action Terraform).

- **Endpoint** : `/admin/token-quotas`
...
```terraform
resource "aisia_admin_token_quotas" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_token_quotas.example.id
}
```
...
> Ressource `resource` : `aisia_admin_token_quotas` — synchronisée avec l'OpenAPI AISIA.
