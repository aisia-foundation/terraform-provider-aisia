...
page_title: "aisia_admin_discovery_scans Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/discovery/scans (resource action Terraform).
  Endpoint : /admin/discovery/scansDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_discovery_scans (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/discovery/scans` (resource action Terraform).

- **Endpoint** : `/admin/discovery/scans`
...
```terraform
resource "aisia_admin_discovery_scans" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_discovery_scans.example.id
}
```
...
> Ressource `resource` : `aisia_admin_discovery_scans` — synchronisée avec l'OpenAPI AISIA.
