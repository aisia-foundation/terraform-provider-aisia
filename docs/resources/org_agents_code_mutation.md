...
page_title: "aisia_org_agents_code_mutation Resource - aisia"
subcategory: ""
description: |-
  Endpoint API AISIA — mutation DELETE exacte /org/agents/{code} (resource action Terraform).
...
# aisia_org_agents_code_mutation (Resource)
...
resource "aisia_org_agents_code_mutation" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_org_agents_code_mutation.run.json)
}
```
...
> Ressource `resource` : `aisia_org_agents_code_mutation` — synchronisée avec l'OpenAPI AISIA.
