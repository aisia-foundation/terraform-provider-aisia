...
page_title: "aisia_admin_config_email_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/config/email (resource action Terraform).
...
# aisia_admin_config_email_action (Resource)
...
resource "aisia_admin_config_email_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_config_email_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_config_email_action` — synchronisée avec l'OpenAPI AISIA.
