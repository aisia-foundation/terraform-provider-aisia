...
page_title: "aisia_admin_network_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/network (resource action Terraform).
...
# aisia_admin_network_action (Resource)
...
resource "aisia_admin_network_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_network_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_network_action` — synchronisée avec l'OpenAPI AISIA.
