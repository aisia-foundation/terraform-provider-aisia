...
page_title: "aisia_admin_rate_limits_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/rate-limits (resource action Terraform).
...
# aisia_admin_rate_limits_action (Resource)
...
resource "aisia_admin_rate_limits_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_rate_limits_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_rate_limits_action` — synchronisée avec l'OpenAPI AISIA.
