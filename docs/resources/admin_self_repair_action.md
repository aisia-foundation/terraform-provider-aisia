...
page_title: "aisia_admin_self_repair_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/self-repair (resource action Terraform).
  Endpoint : /admin/self-repairDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_self_repair_action (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/self-repair` (resource action Terraform).

- **Endpoint** : `/admin/self-repair`
...
resource "aisia_admin_self_repair_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_self_repair_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_self_repair_action` — synchronisée avec l'OpenAPI AISIA.
