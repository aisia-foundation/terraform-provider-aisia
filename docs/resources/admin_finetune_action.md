...
page_title: "aisia_admin_finetune_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/finetune (resource action Terraform).
...
# aisia_admin_finetune_action (Resource)
...
resource "aisia_admin_finetune_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_finetune_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_finetune_action` — synchronisée avec l'OpenAPI AISIA.
