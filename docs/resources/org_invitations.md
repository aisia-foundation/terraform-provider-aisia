...
page_title: "aisia_org_invitations Resource - aisia"
subcategory: ""
description: |-
  Endpoint API AISIA — mutation POST exacte /org/invitations (resource action Terraform).
...
# aisia_org_invitations (Resource)
...
```terraform
resource "aisia_org_invitations" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_org_invitations.example.id
}
```
...
> Ressource `resource` : `aisia_org_invitations` — synchronisée avec l'OpenAPI AISIA.
