...
page_title: "aisia_v1_runs_run_id_stop_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint public API v1 — mutation POST exacte /v1/runs/{run_id}/stop (resource action Terraform).
  Endpoint : /v1/runs/{run_id}/stopDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_v1_runs_run_id_stop_action (Resource)
...
resource "aisia_v1_runs_run_id_stop_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_v1_runs_run_id_stop_action.run.json)
}
```
...
> Ressource `resource` : `aisia_v1_runs_run_id_stop_action` — synchronisée avec l'OpenAPI AISIA.
