...
page_title: "aisia_org_deployments_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint API AISIA — mutation POST exacte /org/deployments (resource action Terraform).
...
# aisia_org_deployments_action (Resource)
...
resource "aisia_org_deployments_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_org_deployments_action.run.json)
}
```
...
> Ressource `resource` : `aisia_org_deployments_action` — synchronisée avec l'OpenAPI AISIA.
