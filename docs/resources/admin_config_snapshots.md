...
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/config/snapshots (resource action Terraform).
  Endpoint : /admin/config/snapshotsDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---
...
- **Endpoint** : `/admin/config/snapshots`
...
```terraform
resource "aisia_admin_config_snapshots" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_config_snapshots.example.id
}
```
...
> Ressource `resource` : `aisia_admin_config_snapshots` — synchronisée avec l'OpenAPI AISIA.
