...
page_title: "aisia_admin_plans Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/plans/ (resource action Terraform).
...
```terraform
resource "aisia_admin_plans" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_plans.example.id
}
```
...
> Ressource `resource` : `aisia_admin_plans` — synchronisée avec l'OpenAPI AISIA.
