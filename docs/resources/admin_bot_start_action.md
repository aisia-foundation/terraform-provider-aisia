...
page_title: "aisia_admin_bot_start_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/bot/start (resource action Terraform).
  Endpoint : /admin/bot/startDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_bot_start_action (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/bot/start` (resource action Terraform).

- **Endpoint** : `/admin/bot/start`
...
resource "aisia_admin_bot_start_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_bot_start_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_bot_start_action` — synchronisée avec l'OpenAPI AISIA.
