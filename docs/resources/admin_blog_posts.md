...
```terraform
resource "aisia_admin_blog_posts" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_blog_posts.example.id
}
```
...
> Ressource `resource` : `aisia_admin_blog_posts` — synchronisée avec l'OpenAPI AISIA.
