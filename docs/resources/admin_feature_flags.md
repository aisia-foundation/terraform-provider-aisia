...
page_title: "aisia_admin_feature_flags Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/feature-flags (resource action Terraform).
...
# aisia_admin_feature_flags (Resource)
...
```terraform
resource "aisia_admin_feature_flags" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_feature_flags.example.id
}
```
...
> Ressource `resource` : `aisia_admin_feature_flags` — synchronisée avec l'OpenAPI AISIA.
