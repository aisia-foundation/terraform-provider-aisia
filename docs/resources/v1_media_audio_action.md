...
page_title: "aisia_v1_media_audio_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint public API v1 — mutation POST exacte /v1/media/audio (resource action Terraform).
  Endpoint : /v1/media/audioDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_v1_media_audio_action (Resource)

Endpoint public API v1 — mutation POST exacte `/v1/media/audio` (resource action Terraform).

- **Endpoint** : `/v1/media/audio`
...
resource "aisia_v1_media_audio_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_v1_media_audio_action.run.json)
}
```
...
> Ressource `resource` : `aisia_v1_media_audio_action` — synchronisée avec l'OpenAPI AISIA.
