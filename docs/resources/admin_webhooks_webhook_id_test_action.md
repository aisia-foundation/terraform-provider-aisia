...
resource "aisia_admin_webhooks_webhook_id_test_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_webhooks_webhook_id_test_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_webhooks_webhook_id_test_action` — synchronisée avec l'OpenAPI AISIA.
