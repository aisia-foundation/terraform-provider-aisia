...
page_title: "aisia_admin_tests_run_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/tests/run (resource action Terraform).
  Endpoint : /admin/tests/runDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_tests_run_action (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/tests/run` (resource action Terraform).

- **Endpoint** : `/admin/tests/run`
...
resource "aisia_admin_tests_run_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_tests_run_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_tests_run_action` — synchronisée avec l'OpenAPI AISIA.
