...
page_title: "aisia_admin_models_sync_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/models/sync (resource action Terraform).
  Endpoint : /admin/models/syncDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_models_sync_action (Resource)
...
resource "aisia_admin_models_sync_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_models_sync_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_models_sync_action` — synchronisée avec l'OpenAPI AISIA.
