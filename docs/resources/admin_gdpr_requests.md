...
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/gdpr/requests (resource action Terraform).
...
```terraform
resource "aisia_admin_gdpr_requests" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_gdpr_requests.example.id
}
```
...
> Ressource `resource` : `aisia_admin_gdpr_requests` — synchronisée avec l'OpenAPI AISIA.
