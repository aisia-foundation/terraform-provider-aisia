...
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/email-templates (resource action Terraform).
...
```terraform
resource "aisia_admin_email_templates" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_email_templates.example.id
}
```
...
> Ressource `resource` : `aisia_admin_email_templates` — synchronisée avec l'OpenAPI AISIA.
