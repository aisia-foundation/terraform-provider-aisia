...
page_title: "aisia_admin_db_migrations Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/db/migrations (resource action Terraform).
  Endpoint : /admin/db/migrationsDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_db_migrations (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/db/migrations` (resource action Terraform).

- **Endpoint** : `/admin/db/migrations`
...
```terraform
resource "aisia_admin_db_migrations" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_db_migrations.example.id
}
```
...
> Ressource `resource` : `aisia_admin_db_migrations` — synchronisée avec l'OpenAPI AISIA.
