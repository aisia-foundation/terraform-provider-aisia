...
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/alerts (resource action Terraform).
...
```terraform
resource "aisia_admin_alerts" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_alerts.example.id
}
```
...
> Ressource `resource` : `aisia_admin_alerts` — synchronisée avec l'OpenAPI AISIA.
