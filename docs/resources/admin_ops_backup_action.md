...
page_title: "aisia_admin_ops_backup_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/ops/backup (resource action Terraform).
  Endpoint : /admin/ops/backupDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_ops_backup_action (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/ops/backup` (resource action Terraform).

- **Endpoint** : `/admin/ops/backup`
...
resource "aisia_admin_ops_backup_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_ops_backup_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_ops_backup_action` — synchronisée avec l'OpenAPI AISIA.
