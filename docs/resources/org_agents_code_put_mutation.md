...
page_title: "aisia_org_agents_code_put_mutation Resource - aisia"
subcategory: ""
description: |-
  Endpoint API AISIA — mutation PUT exacte /org/agents/{code} (resource action Terraform).
  Endpoint : /org/agents/{code}Documentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_org_agents_code_put_mutation (Resource)

Endpoint API AISIA — mutation PUT exacte `/org/agents/{code}` (resource action Terraform).

- **Endpoint** : `/org/agents/{code}`
...
resource "aisia_org_agents_code_put_mutation" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_org_agents_code_put_mutation.run.json)
}
```
...
> Ressource `resource` : `aisia_org_agents_code_put_mutation` — synchronisée avec l'OpenAPI AISIA.
