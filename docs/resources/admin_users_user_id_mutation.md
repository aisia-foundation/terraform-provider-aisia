...
page_title: "aisia_admin_users_user_id_mutation Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation DELETE exacte /admin/users/{user_id} (resource action Terraform).
...
# aisia_admin_users_user_id_mutation (Resource)
...
resource "aisia_admin_users_user_id_mutation" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_users_user_id_mutation.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_users_user_id_mutation` — synchronisée avec l'OpenAPI AISIA.
