...
page_title: "aisia_admin_cache_purge_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/cache/purge (resource action Terraform).
  Endpoint : /admin/cache/purgeDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_cache_purge_action (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/cache/purge` (resource action Terraform).

- **Endpoint** : `/admin/cache/purge`
...
resource "aisia_admin_cache_purge_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_cache_purge_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_cache_purge_action` — synchronisée avec l'OpenAPI AISIA.
