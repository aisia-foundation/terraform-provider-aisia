...
page_title: "aisia_admin_bot_stop_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/bot/stop (resource action Terraform).
  Endpoint : /admin/bot/stopDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_bot_stop_action (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/bot/stop` (resource action Terraform).

- **Endpoint** : `/admin/bot/stop`
...
resource "aisia_admin_bot_stop_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_bot_stop_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_bot_stop_action` — synchronisée avec l'OpenAPI AISIA.
