...
page_title: "aisia_admin_analytics_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/analytics (resource action Terraform).
...
# aisia_admin_analytics_action (Resource)
...
resource "aisia_admin_analytics_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_analytics_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_analytics_action` — synchronisée avec l'OpenAPI AISIA.
