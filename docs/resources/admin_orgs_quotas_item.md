...
page_title: "aisia_admin_orgs_quotas_item Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation PUT exacte /admin/orgs/quotas/{org_id} (resource action Terraform).
  Endpoint : /admin/orgs/quotas/{org_id}Documentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_orgs_quotas_item (Resource)

Endpoint d'administration (Bearer requis) — mutation PUT exacte `/admin/orgs/quotas/{org_id}` (resource action Terraform).

- **Endpoint** : `/admin/orgs/quotas/{org_id}`
...
```terraform
resource "aisia_admin_orgs_quotas_item" "example" {
  body = jsonencode({
...
output "entity_id" {
  value = aisia_admin_orgs_quotas_item.example.id
}
```
...
> Ressource `resource` : `aisia_admin_orgs_quotas_item` — synchronisée avec l'OpenAPI AISIA.
