...
page_title: "aisia_admin_ops_build_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/ops/build (resource action Terraform).
  Endpoint : /admin/ops/buildDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_ops_build_action (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/ops/build` (resource action Terraform).

- **Endpoint** : `/admin/ops/build`
...
resource "aisia_admin_ops_build_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_ops_build_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_ops_build_action` — synchronisée avec l'OpenAPI AISIA.
