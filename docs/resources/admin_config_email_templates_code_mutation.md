...
page_title: "aisia_admin_config_email_templates_code_mutation Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation DELETE exacte /admin/config/email/templates/{code} (resource action Terraform).
...
# aisia_admin_config_email_templates_code_mutation (Resource)
...
resource "aisia_admin_config_email_templates_code_mutation" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_config_email_templates_code_mutation.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_config_email_templates_code_mutation` — synchronisée avec l'OpenAPI AISIA.
