---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "aisia_admin_learning_proposals_proposal_id_apply_action Resource - aisia"
subcategory: ""
description: |-
  Mutation admin POST /admin/learning/proposals/{proposal_id}/apply (idempotent via Terraform, GOV-01).
  Endpoint : /admin/learning/proposals/{proposal_id}/applyDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_learning_proposals_proposal_id_apply_action (Resource)

Mutation admin POST `/admin/learning/proposals/{proposal_id}/apply` (idempotent via Terraform, GOV-01).

- **Endpoint** : `/admin/learning/proposals/{proposal_id}/apply`
- **Documentation produit** : [aisia.fr/docs](https://aisia.fr/docs)
- **Référence API OpenAPI** : [api.aisia.fr/docs](https://api.aisia.fr/docs)

## Example Usage

```terraform
# Mutation admin — adapter le corps JSON au schéma OpenAPI
resource "aisia_admin_learning_proposals_proposal_id_apply_action" "run" {
  body = jsonencode({
    # champs requis : voir https://api.aisia.fr/docs
  })
}

output "result" {
  value = jsondecode(aisia_admin_learning_proposals_proposal_id_apply_action.run.json)
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `body` (String) Corps JSON (ex. `jsonencode({...})`). Clés requises si pathTemplate contient {placeholders}.

### Read-Only

- `id` (String) Identifiant fixe de la mutation.
- `json` (String) Dernière réponse brute de l'API (JSON).

<!-- TF-DOCS-ENRICH:09_publications -->
## Documentation AISIA

- **Documentation produit** : [aisia.fr/docs](https://aisia.fr/docs)
- **Référence API OpenAPI** : [api.aisia.fr/docs](https://api.aisia.fr/docs)
- **Guide d'implémentation Terraform** : [guides/getting-started](guides/getting-started.md)
- **Provider registry** : [aisia-foundation/aisia](https://registry.terraform.io/providers/aisia-foundation/aisia/latest/docs)

> Ressource `resource` : `aisia_admin_learning_proposals_proposal_id_apply_action` — synchronisée avec l'OpenAPI AISIA.
