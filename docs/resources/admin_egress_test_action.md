...
page_title: "aisia_admin_egress_test_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation POST exacte /admin/egress/test (resource action Terraform).
  Endpoint : /admin/egress/testDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_egress_test_action (Resource)

Endpoint d'administration (Bearer requis) — mutation POST exacte `/admin/egress/test` (resource action Terraform).

- **Endpoint** : `/admin/egress/test`
...
resource "aisia_admin_egress_test_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_egress_test_action.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_egress_test_action` — synchronisée avec l'OpenAPI AISIA.
