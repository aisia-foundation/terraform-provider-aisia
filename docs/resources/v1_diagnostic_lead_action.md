...
page_title: "aisia_v1_diagnostic_lead_action Resource - aisia"
subcategory: ""
description: |-
  Endpoint public API v1 — mutation POST exacte /v1/diagnostic/lead (resource action Terraform).
  Endpoint : /v1/diagnostic/leadDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_v1_diagnostic_lead_action (Resource)

Endpoint public API v1 — mutation POST exacte `/v1/diagnostic/lead` (resource action Terraform).

- **Endpoint** : `/v1/diagnostic/lead`
...
resource "aisia_v1_diagnostic_lead_action" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_v1_diagnostic_lead_action.run.json)
}
```
...
> Ressource `resource` : `aisia_v1_diagnostic_lead_action` — synchronisée avec l'OpenAPI AISIA.
