...
page_title: "aisia_admin_users_user_id_role_mutation Resource - aisia"
subcategory: ""
description: |-
  Endpoint d'administration (Bearer requis) — mutation PUT exacte /admin/users/{user_id}/role (resource action Terraform).
  Endpoint : /admin/users/{user_id}/roleDocumentation produit : aisia.fr/docs https://aisia.fr/docsRéférence API OpenAPI : api.aisia.fr/docs https://api.aisia.fr/docs
---

# aisia_admin_users_user_id_role_mutation (Resource)
...
resource "aisia_admin_users_user_id_role_mutation" "run" {
  body = jsonencode({
...
output "result" {
  value = jsondecode(aisia_admin_users_user_id_role_mutation.run.json)
}
```
...
> Ressource `resource` : `aisia_admin_users_user_id_role_mutation` — synchronisée avec l'OpenAPI AISIA.
