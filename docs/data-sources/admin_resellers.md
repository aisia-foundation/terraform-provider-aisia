...
```terraform
data "aisia_admin_resellers" "current" {}

# Exploiter la réponse brute (liste ou objet JSON)
output "admin_resellers_count" {
  value = data.aisia_admin_resellers.current.item_count
}

output "admin_resellers_ids" {
  value = data.aisia_admin_resellers.current.ids
}

output "admin_resellers_payload" {
  value = jsondecode(data.aisia_admin_resellers.current.json)
}
```
...
> Ressource `data source` : `aisia_admin_resellers` — synchronisée avec l'OpenAPI AISIA.
