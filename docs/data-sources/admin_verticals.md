...
```terraform
data "aisia_admin_verticals" "current" {}

# Exploiter la réponse brute (liste ou objet JSON)
output "admin_verticals_count" {
  value = data.aisia_admin_verticals.current.item_count
}

output "admin_verticals_ids" {
  value = data.aisia_admin_verticals.current.ids
}

output "admin_verticals_payload" {
  value = jsondecode(data.aisia_admin_verticals.current.json)
}
```
...
> Ressource `data source` : `aisia_admin_verticals` — synchronisée avec l'OpenAPI AISIA.
